// Clearance Bundle Image Creator V2.0 (PB) Js
$(document).ready(function () {
  $(".clearance_bundle_image_creator").on(
    "click",
    "#render_image_btn",
    function () {
      renderNewView();
...
function getXML(sku_list) {
  ...
  $.each(sku_list, function (index, sku) {
    // var template =
    //   "https://www.dfs.co.uk/webapp/wcs/stores/servlet/GetAmplienceProductDataCmd?sku=" +
    //   sku;
...
function renderImage(images) {
  var template =
    "https://x1.adis.ws/v1/media/graphics/i/dfs/clearance_bundle_template" +
    images.length;
  var params = "?fmt=jpg&";
  ...
  if (images.length === 4) {
    ...
    $(
      ".attributes.main_image, .attributes.left_image, .attributes.middle_image, .attributes.right_image"
    ).addClass("active");
  }
  if (images.length === 3) {
    width = $("input#width").val();
    ...
    $(
      ".attributes.main_image, .attributes.left_image, .attributes.middle_image"
    ).addClass("active");
  }
  if (images.length === 2) {
    width = $("input#width").val();
